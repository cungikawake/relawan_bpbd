                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body">
                                <a href="{{ route('dashboard.persyaratan.create') }}" class="btn btn-info btn-icon btn-sm mr-1 mb-1"><i class="la la-plus"></i> Buat Baru </a>
                                            ...
                                            @if(count($datas) > 0)
                                                @foreach($datas as $key => $data)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="{{route('dashboard.persyaratan.edit', $data->id)}}" class="btn btn-icon btn-warning btn-sm"><i class="ft-edit"></i></a>
                                                            <button type="button" class="btn btn-icon btn-danger btn-sm delete" data-id="{{$data->id}}"><i class="la la-ban"></i></button>
                                                            <form action="{{route('dashboard.persyaratan.destroy', $data->id)}}" id="delete-{{$data->id}}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                            </form>
                                                        </td>
